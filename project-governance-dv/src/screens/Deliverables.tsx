...
export default function Deliverables() {
  ...
  const filteredDeliverables = useMemo(() => {
    ...
    const normalizedOwnerFilter = ownerFilter === 'All' ? 'All' : normalizePersonKey(ownerFilter)
    const normalizedWorkstreamFilter = workstreamFilter === 'All' ? 'All' : normalizeKey(workstreamFilter)
    let items = deliverables.filter((d: Deliverable) => {
      if (statusFilter !== 'All' && d.status !== statusFilter) return false
      if (riskFilter !== 'All' && d.risk !== riskFilter) return false
      if (
        normalizedWorkstreamFilter !== 'All' &&
        normalizeKey(resolveWorkstreamDisplay(d.workstreamId, workstreamById, workstreams)) !==
          normalizedWorkstreamFilter
      ) {
        return false
      }
      if (
        normalizedOwnerFilter !== 'All' &&
        normalizePersonKey(resolveOwnerDisplay(d.ownerId, staffById, staffByEmail, staffByName)) !==
